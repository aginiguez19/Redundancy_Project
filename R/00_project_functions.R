...
# Form 1 HCSF Function  ----------------------------------------------------------
hcsf <- function(matrix, loading = .9){
  # Lambda
  ...
  theta <- diag(.19, nrow = dimensions_theta[1], ncol = dimensions_theta[2])
...
hcdf <- function(matrix, loading = .9){
  # Lambda
  dimensions <- dim(matrix)
  lambda_matrix <- diag(x = loading, nrow = dimensions[1] + 1 , ncol = dimensions[2])
  last_element <- nrow(lambda_matrix) * ncol(lambda_matrix)
  ...
  theta <- diag(.19, nrow = dimensions_theta[1], ncol = dimensions_theta[2])
  redun_mat <- redun_mat + theta
  redun_mat[1:nv,nv+1] <- -1*redun_mat[1:nv,nv+1]
  return(redun_mat)
}
