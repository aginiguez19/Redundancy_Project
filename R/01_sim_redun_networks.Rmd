...
local_metrics <- array(NA, dim = c(nsim, (2*nv + 1), 4),
                 dimnames = list(NULL, c(paste0("P", 1:(nv-1)), "Target", paste0("P", 1:(nv-1), ".r"), "Target.r", "Clone"),  c("Expected Influence","Strength", "Closeness", "Betweenness")))

global_metrics = array(NA, dim = c(nsim, 2, 2),
                       dimnames = list(NULL, c("True Network", "Redundant Network"), c("APL", "Weighted Density")))



for (i in 1:nsim){
# Correlation matrix of the latent variables
  lat_corr <- corr_gen(nv = 4, EF = EF, edge.probability = ep)
  
# Correlation matrix of the indicators with the inclusion of the redundant indicator
  redun_corr <- ind_corr(matrix = lat_corr,loadings = loading, clone_loading = .9,
                         redundancy = TRUE)
...
# Create a block for easier centrality computation 
  ...
  the_g <- rbind(one, two)
  the_g <- graph_from_adjacency_matrix(the_g, mode = "undirected", weighted = TRUE)
  tpcor_graph = graph_from_adjacency_matrix(true_pcor, mode = "undirected", weighted = TRUE)
  rpcor_graph = graph_from_adjacency_matrix(redun_pcor, mode = "undirected", weighted = TRUE) 
  

# Get centrality metrics  
  local_metrics[i,, "Expected Influence"] <- strength(the_g)
  local_metrics[i,,"Strength"] <- strength(the_g, weights = abs(E(the_g)$weight))
  local_metrics[i,,"Closeness"] <- closeness(the_g, weights = 1/round(abs(E(the_g)$weight),2), normalized = TRUE)
  local_metrics[i,,"Betweenness"] <- betweenness(the_g, weights = 1/(abs(E(the_g)$weight)), normalized = FALSE)
  ...
  global_metrics[i, "True Network", "Weighted Density"] <- weighted_density(igrph.object = tpcor_graph, abs = TRUE)
  global_metrics[i, "Redundant Network", "Weighted Density"] <- weighted_density(igrph.object = rpcor_graph, abs = TRUE)
}
...
1) Continue modifying simulation code to include 
